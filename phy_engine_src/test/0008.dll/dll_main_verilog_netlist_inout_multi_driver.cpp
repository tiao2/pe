...
{
    // Circuit:
    //   INPUT ext, INPUT d -> verilog module with inout x (driven by d) -> OUTPUT x
    int elements[] = {PE_INPUT, PE_INPUT, PE_VERILOG_NETLIST, PE_OUTPUT};

    int wires[] = {
        0, 0, 2, 0,  // ext.o -> vmod.x (inout)
        1, 0, 2, 1,  // d.o -> vmod.d
        2, 0, 3, 0,  // vmod.x -> out.i
    };

    // properties: INPUT states for ext, d
...
module top(inout x, input d);
  assign x = d;
    ...
    auto x_state = [&]() -> ::phy_engine::model::digital_node_statement_t
    {
        auto* c = static_cast<::phy_engine::circult*>(cptr);
        auto* out = ::phy_engine::netlist::get_model(c->get_netlist(), {vec_pos[3], chunk_pos[3]});
    ...
    // ext=0, d=1 => x resolves to X (inout + external drive conflict).
    if(!set_in(0, 0) || !set_in(1, 1)) { destroy_circuit(cptr, vec_pos, chunk_pos); return 6; }
    if(circuit_digital_clk(cptr) != 0) { destroy_circuit(cptr, vec_pos, chunk_pos); return 7; }
    if(x_state() != ::phy_engine::model::digital_node_statement_t::indeterminate_state)
    {
        destroy_circuit(cptr, vec_pos, chunk_pos);
        return 8;
    }
