...
{
    // Circuit:
    //   INPUT clk, INPUT d -> verilog module dff (synthesized) -> OUTPUT q
    int elements[] = {PE_INPUT, PE_INPUT, PE_VERILOG_NETLIST, PE_OUTPUT};

    int wires[] = {
        0, 0, 2, 0,  // clk.o -> vmod.clk
        1, 0, 2, 1,  // d.o -> vmod.d
        2, 2, 3, 0,  // vmod.q -> out.i
    };

    // properties: INPUT state for clk, INPUT state for d
...
module top(input clk, input d, output reg q);
  always @(posedge clk) begin
    ...
    // init: clk=0,d=0
    if(!set_in(1, 0) || !clk0()) { destroy_circuit(cptr, vec_pos, chunk_pos); return 6; }

    // set d=1 then rising edge => q=1
    if(!set_in(1, 1) || !clk1()) { destroy_circuit(cptr, vec_pos, chunk_pos); return 7; }
    if(!sample_q()) { destroy_circuit(cptr, vec_pos, chunk_pos); return 8; }

    // falling edge shouldn't change q
    if(!clk0()) { destroy_circuit(cptr, vec_pos, chunk_pos); return 9; }
    if(!sample_q()) { destroy_circuit(cptr, vec_pos, chunk_pos); return 10; }

    // set d=0 then rising edge => q=0
    if(!set_in(1, 0) || !clk1()) { destroy_circuit(cptr, vec_pos, chunk_pos); return 11; }
    if(sample_q()) { destroy_circuit(cptr, vec_pos, chunk_pos); return 12; }
