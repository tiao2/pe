...
{
    // Circuit:
    //   INPUT a, INPUT b -> verilog module with multiple drivers -> OUTPUT y
    ...
    // properties: INPUT states for a, b
...
module top(input a, input b, output y);
  ...
  assign y = b;
    ...
    auto y_state = [&]() -> ::phy_engine::model::digital_node_statement_t
    {
    ...
    // a=0,b=1 => y resolves to X (multi-driver conflict).
    ...
    if(y_state() != ::phy_engine::model::digital_node_statement_t::indeterminate_state)
    {
        destroy_circuit(cptr, vec_pos, chunk_pos);
        return 8;
    }
