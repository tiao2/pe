...
int main()
{
    ...
    auto const* mod = find_module(d, u8sv{u8"fp16_addsub_unit"});
    ...
    for(auto const& t : cases)
    {
        ...
        if(!set_uN_vec_value(*mod, st, u8sv{u8"sub"}, t.sub ^ 1u, 1)) { return 6; }
        commit_prev(st);

        if(!set_uN_vec_value(*mod, st, u8sv{u8"a"}, t.a, 16)) { return 7; }
        if(!set_uN_vec_value(*mod, st, u8sv{u8"b"}, t.b, 16)) { return 8; }
        if(!set_uN_vec_value(*mod, st, u8sv{u8"sub"}, t.sub, 1)) { return 9; }

        simulate(st, 5, false);

        std::uint64_t y{};
        if(!read_uN_vec(*mod, st, u8sv{u8"y"}, y, 16)) { return 10; }
        auto const got = static_cast<std::uint16_t>(y);
        if(got != t.exp)
        {
            std::uint64_t exp_a_f{}, exp_b_f{}, frac_a{}, frac_b{}, exp_a_adj{}, exp_b_adj{}, exp_res{};
            std::uint64_t mant_a{}, mant_b{}, ext_a{}, ext_b{}, mant_res_ext{};
            ...
            (void)read_uN_vec_any(*mod, st, u8sv{u8"exp_res"}, exp_res, 6);
            (void)read_uN_vec_any(*mod, st, u8sv{u8"mant_a"}, mant_a, 11);
            (void)read_uN_vec_any(*mod, st, u8sv{u8"mant_b"}, mant_b, 11);
            (void)read_uN_vec_any(*mod, st, u8sv{u8"ext_a"}, ext_a, 15);
            (void)read_uN_vec_any(*mod, st, u8sv{u8"ext_b"}, ext_b, 15);
            (void)read_uN_vec_any(*mod, st, u8sv{u8"mant_res_ext"}, mant_res_ext, 15);
            ...
            std::fprintf(stderr,
                         "addsub mismatch a=0x%04x b=0x%04x sub=%u got=0x%04x exp=0x%04x exp_a_f=%llu exp_b_f=%llu frac_a=0x%03llx frac_b=0x%03llx zero_a=%llu zero_b=%llu nan_a=%llu nan_b=%llu inf_a=%llu inf_b=%llu exp_a_adj=%llu exp_b_adj=%llu exp_res=%llu mant_a=0x%03llx mant_b=0x%03llx ext_a=0x%04llx ext_b=0x%04llx mant_res=0x%04llx\n",
                         t.a,
                         t.b,
                         t.sub,
                         got,
