...
module top(input [7:0] in8, output reg [7:0] out8);
  reg [7:0] tmp;
  reg [2:0] i;
  always @* begin
    tmp = in8;
    ...
    if(!set_uN_vec_value(*m, st, u8sv{u8"in8"}, 0u, 8)) { return 3; }
    commit_prev(st);
    if(!set_uN_vec_value(*m, st, u8sv{u8"in8"}, 0x30u, 8)) { return 4; }
    simulate(st, 2, false);

    std::uint64_t out{};
    if(!read_uN_vec(*m, st, u8sv{u8"out8"}, out, 8)) { return 5; }
    std::fprintf(stderr, "out8=0x%02llx\n", static_cast<unsigned long long>(out));
    return (out == 0x18u) ? 0 : 6;
}
