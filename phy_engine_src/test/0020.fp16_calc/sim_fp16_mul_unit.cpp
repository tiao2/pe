...
int main()
{
    auto const src_path = std::filesystem::path(__FILE__).parent_path() / "fp16_mul.v";
    ...
    auto const* mod = find_module(d, u8sv{u8"fp16_mul_unit"});
    ...
    constexpr tc cases[] = {
        {0x3E00u, 0x4000u, 0x4200u},  // 1.5 * 2.0 = 3.0
        {0xBC00u, 0xC000u, 0x4000u},  // -1.0 * -2.0 = 2.0
    };
    ...
    for(auto const& t : cases)
    {
        ...
        if(got != t.exp)
        {
            ...
            std::uint64_t mant_a{}, mant_b{}, prod{}, mant_ext{};
            ...
            (void)read_uN_vec_any(*mod, st, u8sv{u8"prod"}, prod, 22);
            ...
            std::fprintf(stderr,
                         "mul mismatch a=0x%04x b=0x%04x got=0x%04x exp=0x%04x exp_a_f=%llu exp_b_f=%llu frac_a=0x%03llx frac_b=0x%03llx zero_a=%llu zero_b=%llu nan_a=%llu nan_b=%llu inf_a=%llu inf_b=%llu exp_a_adj=%llu exp_b_adj=%llu exp_res=%llu mant_a=0x%03llx mant_b=0x%03llx prod=0x%06llx mant_ext=0x%04llx\n",
                         t.a,
                         t.b,
                         got,
                         t.exp,
