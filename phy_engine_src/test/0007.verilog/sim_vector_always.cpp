...
module v_not(input [3:0] a, output reg [3:0] y);
  always @* begin
    y = ~a;
  end
endmodule
)";
    constexpr char8_t top_name[] = u8"v_not";
    ...
    // a = 0101 => y = 1010
    set_in(a3, ::phy_engine::model::digital_node_statement_t::false_state);
