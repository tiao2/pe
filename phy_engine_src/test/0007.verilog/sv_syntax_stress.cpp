...
int main()
{
    ...
    // SystemVerilog syntax stress test (single file).
    // Goal: exercise parsing/compilation front-end coverage (not functional correctness).
...
`endif
  ...
  always_ff @(posedge clk or negedge rst_n) begin
    ...
    if(cr.errors.empty()) { return 0; }

    // Best-effort diagnostics to help pin-point unsupported syntax.
    auto to_string = [](::fast_io::u8string const& s) -> ::std::string {
        return ::std::string(reinterpret_cast<char const*>(s.data()), s.size());
    };

    ::std::cerr << "sv_syntax_stress: compile errors = " << cr.errors.size() << "\n";
    ::std::size_t const n{cr.errors.size() < 20 ? cr.errors.size() : 20};
    for(::std::size_t i{}; i < n; ++i)
    {
        auto const& e = cr.errors.index_unchecked(i);
        ::std::cerr << "  [" << i << "] line=" << e.line << " col=" << e.column << ": " << to_string(e.message) << "\n";
    }
    return 1;
}
