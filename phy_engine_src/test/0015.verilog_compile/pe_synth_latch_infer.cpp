...
module top(input a, input en, output reg y);
  always @* begin
    ...
    auto [in_en, p1] = ::phy_engine::netlist::add_model(nl, ::phy_engine::model::INPUT{.outputA = ::phy_engine::model::digital_node_statement_t::false_state});
    auto [out_y, p2] = ::phy_engine::netlist::add_model(nl, ::phy_engine::model::OUTPUT{});
    (void)p0;
    (void)p1;
    (void)p2;
    if(in_a == nullptr || in_en == nullptr || out_y == nullptr) { return 4; }

    if(!::phy_engine::netlist::add_to_node(nl, *in_a, 0, *ports[0])) { return 5; }
    if(!::phy_engine::netlist::add_to_node(nl, *in_en, 0, *ports[1])) { return 6; }
    ...
    if(!cct.analyze()) { return 9; }
    auto tick = [&]() noexcept { cct.digital_clk(); };

    // Write a known state: en=1, a=0 => y=0.
    (void)in_en->ptr->set_attribute(0, dv(::phy_engine::model::digital_node_statement_t::true_state));
    (void)in_a->ptr->set_attribute(0, dv(::phy_engine::model::digital_node_statement_t::false_state));
    tick();
    if(ports[2]->node_information.dn.state != ::phy_engine::model::digital_node_statement_t::false_state) { return 10; }

    // Hold: en=0, a=1 => y stays 0.
    (void)in_en->ptr->set_attribute(0, dv(::phy_engine::model::digital_node_statement_t::false_state));
    (void)in_a->ptr->set_attribute(0, dv(::phy_engine::model::digital_node_statement_t::true_state));
    tick();
    if(ports[2]->node_information.dn.state != ::phy_engine::model::digital_node_statement_t::false_state) { return 11; }

    // Update: en=1 => y becomes 1.
    (void)in_en->ptr->set_attribute(0, dv(::phy_engine::model::digital_node_statement_t::true_state));
    tick();
    if(ports[2]->node_information.dn.state != ::phy_engine::model::digital_node_statement_t::true_state) { return 12; }
