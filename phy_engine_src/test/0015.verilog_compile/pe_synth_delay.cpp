...
module top(input a, output reg y);
  always @* begin
    ...
    if(!cct.analyze()) { return 8; }

    auto tick = [&]() noexcept { cct.digital_clk(); };

    // a=0, after any ticks y should remain 0.
    tick();
    tick();
    tick();
    if(ports[1]->node_information.dn.state != ::phy_engine::model::digital_node_statement_t::false_state) { return 9; }

    // a=1; y updates after 2 ticks.
    (void)in_a->ptr->set_attribute(0, dv(::phy_engine::model::digital_node_statement_t::true_state));
    tick();
    if(ports[1]->node_information.dn.state != ::phy_engine::model::digital_node_statement_t::false_state)
    {
        ::fast_io::println("delay tick1 mismatch: a=", ports[0]->node_information.dn.state, " y=", ports[1]->node_information.dn.state);
        return 10;
    }
    tick();
    if(ports[1]->node_information.dn.state != ::phy_engine::model::digital_node_statement_t::false_state)
    {
        ::fast_io::println("delay tick2 mismatch: a=", ports[0]->node_information.dn.state, " y=", ports[1]->node_information.dn.state);
        return 11;
    }
    tick();
    if(ports[1]->node_information.dn.state != ::phy_engine::model::digital_node_statement_t::true_state)
    {
        ::fast_io::println("delay tick3 mismatch: a=", ports[0]->node_information.dn.state, " y=", ports[1]->node_information.dn.state);
        return 12;
    }
