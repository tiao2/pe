...
module top(input [3:0] a, output reg [3:0] y);
  ...
  always @* begin
    ...
    set_in(in_a[1], true);   // a[2]
    set_in(in_a[2], false);  // a[1]
    ...
    if(ports[y_idx[1]]->node_information.dn.state != ::phy_engine::model::digital_node_statement_t::true_state) { return 13; }
    if(ports[y_idx[2]]->node_information.dn.state != ::phy_engine::model::digital_node_statement_t::false_state) { return 14; }
