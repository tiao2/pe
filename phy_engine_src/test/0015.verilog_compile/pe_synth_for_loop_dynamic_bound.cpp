...
module top(input [1:0] n, input [3:0] a, output reg [3:0] y);
  ...
  always @* begin
    ...
    // n = 2, a = 1010 => y[1:0]=a[1:0]=10, others 0 => 0010
    set_in(in_n1, ::phy_engine::model::digital_node_statement_t::true_state);
    set_in(in_n0, ::phy_engine::model::digital_node_statement_t::false_state);

    set_in(in_a[0], ::phy_engine::model::digital_node_statement_t::true_state);   // a[3]
    set_in(in_a[1], ::phy_engine::model::digital_node_statement_t::false_state);  // a[2]
    set_in(in_a[2], ::phy_engine::model::digital_node_statement_t::true_state);   // a[1]
    set_in(in_a[3], ::phy_engine::model::digital_node_statement_t::false_state);  // a[0]

    settle();

    if(ports[y_idx[0]]->node_information.dn.state != ::phy_engine::model::digital_node_statement_t::false_state) { return 16; }
    if(ports[y_idx[1]]->node_information.dn.state != ::phy_engine::model::digital_node_statement_t::false_state) { return 17; }
    if(ports[y_idx[2]]->node_information.dn.state != ::phy_engine::model::digital_node_statement_t::true_state) { return 18; }
    if(ports[y_idx[3]]->node_information.dn.state != ::phy_engine::model::digital_node_statement_t::false_state) { return 19; }

    // n = 0 => y remains 0
    set_in(in_n1, ::phy_engine::model::digital_node_statement_t::false_state);
    set_in(in_n0, ::phy_engine::model::digital_node_statement_t::false_state);
    settle();
    for(auto idx: y_idx)
    {
        if(ports[idx]->node_information.dn.state != ::phy_engine::model::digital_node_statement_t::false_state) { return 20; }
    }
