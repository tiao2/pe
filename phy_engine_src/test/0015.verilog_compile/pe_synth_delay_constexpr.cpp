...
module top #(parameter integer P=1) (input a, output reg y);
  always @* begin
    #(P + 1) y = a;
  end
endmodule
    ...
    // P=1 => #(P+1) == #2. Start with a=0 and tick a few times.
    tick();
    tick();
    tick();
    ...
    tick();
    if(ports[1]->node_information.dn.state != ::phy_engine::model::digital_node_statement_t::false_state) { return 10; }
    tick();
    if(ports[1]->node_information.dn.state != ::phy_engine::model::digital_node_statement_t::false_state) { return 11; }
    tick();
    if(ports[1]->node_information.dn.state != ::phy_engine::model::digital_node_statement_t::true_state) { return 12; }
