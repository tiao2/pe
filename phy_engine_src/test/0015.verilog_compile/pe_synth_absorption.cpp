...
std::optional<run_result> run_once(::fast_io::u8string_view src, std::uint8_t opt_level) noexcept
{
    ...
    if(!idx_a || !idx_b || !idx_y1 || !idx_y2 || !idx_y3 || !idx_y4) { return std::nullopt; }
    ...
    for(std::uint32_t mask{}; mask < 4u; ++mask)
    {
        ...
        bool const exp_y1 = a;
        bool const exp_y2 = a;
        bool const exp_y3 = (a && b);
        bool const exp_y4 = (a || b);
        if(*y1 != exp_y1 || *y2 != exp_y2 || *y3 != exp_y3 || *y4 != exp_y4)
        {
            std::fprintf(stderr,
                         "mismatch at O%u (mask=%u): got y1=%d y2=%d y3=%d y4=%d, expected y1=%d y2=%d y3=%d y4=%d\n",
                         static_cast<unsigned>(opt_level),
                         mask,
                         *y1 ? 1 : 0,
                         *y2 ? 1 : 0,
...
module top(
  ...
  assign y1 = a & (a | b); // absorption
  assign y2 = a | (a & b); // absorption
  assign y3 = a & (a & b); // redundant AND level
  assign y4 = a | (a | b); // redundant OR level
endmodule
)";
