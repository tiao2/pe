...
module top(input a, input b, input sel, output reg y);
  always @* begin
    if(sel) y = a;
    else y = b;
  end
    ...
    auto [in_sel, p2] = ::phy_engine::netlist::add_model(nl, ::phy_engine::model::INPUT{.outputA = ::phy_engine::model::digital_node_statement_t::false_state});
    auto [out_y, p3] = ::phy_engine::netlist::add_model(nl, ::phy_engine::model::OUTPUT{});
    (void)p0;
    (void)p1;
    (void)p2;
    (void)p3;
    if(in_a == nullptr || in_b == nullptr || in_sel == nullptr || out_y == nullptr) { return 4; }

    if(!::phy_engine::netlist::add_to_node(nl, *in_a, 0, *ports[0])) { return 5; }
    if(!::phy_engine::netlist::add_to_node(nl, *in_b, 0, *ports[1])) { return 6; }
    if(!::phy_engine::netlist::add_to_node(nl, *in_sel, 0, *ports[2])) { return 7; }
    if(!::phy_engine::netlist::add_to_node(nl, *out_y, 0, *ports[3])) { return 8; }

    ::phy_engine::verilog::digital::pe_synth_error err{};
    if(!::phy_engine::verilog::digital::synthesize_to_pe_netlist(nl, top_inst, ports, &err, {.allow_inout = true, .allow_multi_driver = true})) { return 9; }
    ...
    (void)in_sel->ptr->set_attribute(0, dv(::phy_engine::model::digital_node_statement_t::false_state));
    c.digital_clk();
    if(ports[3]->node_information.dn.state != ::phy_engine::model::digital_node_statement_t::false_state) { return 11; }

    // sel=1 => y=a
    (void)in_sel->ptr->set_attribute(0, dv(::phy_engine::model::digital_node_statement_t::true_state));
    c.digital_clk();
