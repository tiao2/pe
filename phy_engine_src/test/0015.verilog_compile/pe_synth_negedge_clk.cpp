...
module top(input clk, input d, output reg q);
  always @(negedge clk) begin
    ...
    auto [in_clk, p0] = ::phy_engine::netlist::add_model(nl, ::phy_engine::model::INPUT{.outputA = ::phy_engine::model::digital_node_statement_t::true_state});
    ...
    auto [out_q, p2] = ::phy_engine::netlist::add_model(nl, ::phy_engine::model::OUTPUT{});
    (void)p0;
    (void)p1;
    (void)p2;
    if(in_clk == nullptr || in_d == nullptr || out_q == nullptr) { return 4; }
    ...
    if(!::phy_engine::netlist::add_to_node(nl, *out_q, 0, *ports[2])) { return 7; }
    ...
    // Settle initial network.
    tick();

    // clk starts at 1, set d=1, no capture yet.
    (void)in_d->ptr->set_attribute(0, dv(::phy_engine::model::digital_node_statement_t::true_state));
    tick();

    // falling edge clk: 1 -> 0 => capture => q=1
    (void)in_clk->ptr->set_attribute(0, dv(::phy_engine::model::digital_node_statement_t::false_state));
    tick();
    // Some update-table propagation depends on node ordering; a second tick ensures the inverted clock edge reaches the DFF.
    tick();
