...
std::optional<run_result> run_once(::fast_io::u8string_view src, std::uint8_t opt_level) noexcept
{
    ...
    if(!idx_a || !idx_b || !idx_c || !idx_d || !idx_e || !idx_y) { return std::nullopt; }
    ...
    for(std::uint32_t mask{}; mask < 32u; ++mask)
    {
        ...
        bool const exp = (a || (b && c_in && d && e));
        if(*y != exp)
        {
            std::fprintf(stderr, "mismatch at O%u (mask=%u): got y=%d expected y=%d\n", static_cast<unsigned>(opt_level), mask, *y ? 1 : 0, exp ? 1 : 0);
            return std::nullopt;
        }
    }
