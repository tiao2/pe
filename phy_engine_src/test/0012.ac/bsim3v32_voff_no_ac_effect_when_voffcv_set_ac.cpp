...
static double run_case(double voff) noexcept
{
    ...
    // Drain at 0.6V DC (AC ground), measure its branch current.
    auto [vd_src, vd_pos] = add_model(nl, ::phy_engine::model::VDC{.V = 0.6});
    ...
    auto [vg_dc, vgdc_pos] = add_model(nl, ::phy_engine::model::VDC{.V = 1.2});
    ...
    auto const idx_vsat = find_attr(m1, "vsat");

    if(idx_w == SIZE_MAX || idx_l == SIZE_MAX || idx_tox == SIZE_MAX || idx_toxm == SIZE_MAX || idx_vth0 == SIZE_MAX || idx_phi == SIZE_MAX ||
       idx_capmod == SIZE_MAX || idx_xpart == SIZE_MAX || idx_voff == SIZE_MAX || idx_voffcv == SIZE_MAX)
    {
    ...
    (void)m1->ptr->set_attribute(idx_xpart, {.d{0.5}, .type{::phy_engine::model::variant_type::d}});

    // Ensure C/V is controlled by voffcv; voff should not perturb AC when voffcv is finite.
    (void)m1->ptr->set_attribute(idx_voffcv, {.d{0.0}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_voff, {.d{voff}, .type{::phy_engine::model::variant_type::d}});

    // Make the saturation boundary sensitive (avoid accidental cancellations).
    if(idx_mobmod != SIZE_MAX) { (void)m1->ptr->set_attribute(idx_mobmod, {.d{3.0}, .type{::phy_engine::model::variant_type::d}}); }
    if(idx_ua != SIZE_MAX) { (void)m1->ptr->set_attribute(idx_ua, {.d{2e-9}, .type{::phy_engine::model::variant_type::d}}); }
    if(idx_vsat != SIZE_MAX) { (void)m1->ptr->set_attribute(idx_vsat, {.d{1e4}, .type{::phy_engine::model::variant_type::d}}); }

    // Remove fixed/overlap caps so the intrinsic charge model dominates.
...
int main()
{
    ...
    double const i1 = run_case(0.4);
    if(!std::isfinite(i0) || !std::isfinite(i1)) { return 1; }
    if(!(i0 > 0.0)) { return 2; }

    double const denom = std::max({1e-12, std::abs(i0), std::abs(i1)});
    if(!(std::abs(i1 - i0) <= 1e-8 * denom)) { return 3; }
    return 0;
}
