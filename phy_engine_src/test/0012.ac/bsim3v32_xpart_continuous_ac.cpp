...
static double run_case(double xpart) noexcept
{
    ...
    // Drain at 2V DC (AC ground), measure its branch current.
...
int main()
{
    // With continuous xpart interpolation, intermediate xpart should yield intermediate drain displacement current.
    double const id_x0 = run_case(0.0);
    double const id_x025 = run_case(0.25);
    double const id_x05 = run_case(0.5);
    if(!std::isfinite(id_x0) || !std::isfinite(id_x025) || !std::isfinite(id_x05)) { return 1; }
    if(!(id_x0 > 0.0)) { return 2; }

    if(!(id_x025 > id_x0 * 1.02)) { return 3; }
    if(!(id_x05 > id_x025 * 1.02)) { return 4; }
    return 0;
}
