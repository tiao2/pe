...
int main()
{
    // Smoke: ACOP + single-point AC with BSIM3v3.2 (capMod!=0 C-matrix path).
    ...
    // NMOS: D=drain, G=gate, S=GND, B=GND. Default capMod is nonzero, enabling the C-matrix path.
    ...
    auto const vd = n_drain.node_information.an.voltage;

    if(!std::isfinite(vg.real()) || !std::isfinite(vg.imag())) { return 2; }
    if(!std::isfinite(vd.real()) || !std::isfinite(vd.imag())) { return 3; }

    // Gate magnitude should be close to the VAC amplitude.
    double const mag_g = std::abs(vg);
    if(!(mag_g > 0.9 && mag_g < 1.1)) { return 4; }
