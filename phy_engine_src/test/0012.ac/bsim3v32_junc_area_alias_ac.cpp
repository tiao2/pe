...
static std::complex<double> run_case(double ad_m2) noexcept
{
    ...
    // Gate held at 0V, so channel is (mostly) off; we want junction depletion C to dominate.
    ...
    // Enable depletion capacitance and set drain junction area using the SPICE-style alias "ad".
    auto const idx_cj = find_attr(m1, "cj");
    auto const idx_ad = find_attr(m1, "ad");
    if(idx_cj == SIZE_MAX || idx_ad == SIZE_MAX) { return {NAN, NAN}; }
    (void)m1->ptr->set_attribute(idx_cj, {.d{2e-3}, .type{::phy_engine::model::variant_type::d}}); // F/m^2
    (void)m1->ptr->set_attribute(idx_ad, {.d{ad_m2}, .type{::phy_engine::model::variant_type::d}});
...
int main()
{
    // Larger junction area => larger depletion capacitance => larger |Im(Iac)|.
    auto const i_small = run_case(1e-12);
    auto const i_big = run_case(1e-10);
    ...
    if(!(std::abs(i_big.imag()) > std::abs(i_small.imag()) * 5.0)) { return 3; }
    return 0;
}
