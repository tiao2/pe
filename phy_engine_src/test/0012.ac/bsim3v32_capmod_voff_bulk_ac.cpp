...
static double run_case(double voff) noexcept
{
    ...
    // Gate AC excitation: VAC=1V from ng to ng_bias (so AC is referenced to AC-grounded bias node).
    ...
    if(idx_w == SIZE_MAX || idx_l == SIZE_MAX || idx_tox == SIZE_MAX || idx_toxm == SIZE_MAX || idx_vth0 == SIZE_MAX || idx_phi == SIZE_MAX ||
       idx_capmod == SIZE_MAX || idx_voff == SIZE_MAX || idx_cgso == SIZE_MAX || idx_cgdo == SIZE_MAX || idx_cgbo == SIZE_MAX || idx_cgs == SIZE_MAX ||
       idx_cgd == SIZE_MAX || idx_cgb == SIZE_MAX)
    {
    ...
    // Bulk displacement current due to gate excitation.
...
int main()
{
    ...
    // - voff=0: mild inversion -> weaker gate-bulk coupling
    // - voff=0.2: effectively cutoff -> stronger gate-bulk coupling
    double const ib_voff0 = run_case(0.0);
    double const ib_voff2 = run_case(0.2);

    if(!std::isfinite(ib_voff0) || !std::isfinite(ib_voff2)) { return 1; }
    if(!(ib_voff2 > ib_voff0 * 1.5)) { return 2; }
    return 0;
}
