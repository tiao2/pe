...
static std::complex<double> run_case(bool drive_drain, junc_params jp) noexcept
{
    ...
    auto const idx_cjsws = find_attr(m1, "cjsws");
    ...
    auto const idx_pbswd = find_attr(m1, "pbswd");
    auto const idx_pbsws = find_attr(m1, "pbsws");
    if(idx_tox == SIZE_MAX || idx_cj == SIZE_MAX || idx_cjsw == SIZE_MAX || idx_cjswg == SIZE_MAX || idx_cjswd == SIZE_MAX || idx_cjsws == SIZE_MAX ||
       idx_pd == SIZE_MAX || idx_ps == SIZE_MAX || idx_ad == SIZE_MAX || idx_as == SIZE_MAX || idx_pb == SIZE_MAX || idx_pbsw == SIZE_MAX ||
       idx_pbswd == SIZE_MAX || idx_pbsws == SIZE_MAX)
    {
        return {NAN, NAN};
    }
    ...
    // Isolate sidewall depletion capacitance only.
    ...
    (void)m1->ptr->set_attribute(idx_cjswg, {.d{0.0}, .type{::phy_engine::model::variant_type::d}});

    (void)m1->ptr->set_attribute(idx_pd, {.d{1e-5}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_ps, {.d{1e-5}, .type{::phy_engine::model::variant_type::d}});

    (void)m1->ptr->set_attribute(idx_cjswd, {.d{jp.cjswd}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_cjsws, {.d{jp.cjsws}, .type{::phy_engine::model::variant_type::d}});

    // Disable global pbsw so per-junction overrides must take effect.
    (void)m1->ptr->set_attribute(idx_pb, {.d{1.0}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_pbsw, {.d{0.0}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_pbswd, {.d{jp.pbswd}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_pbsws, {.d{jp.pbsws}, .type{::phy_engine::model::variant_type::d}});
...
int main()
{
    // 1) Per-junction sidewall cap density: cjswd affects only drain junction, cjsws only source junction.
    junc_params p1{.cjswd{4e-6}, .cjsws{1e-6}, .pbswd{0.0}, .pbsws{0.0}};
    ...
    // 2) Per-junction sidewall potential: pbswd/pbsws override pb for sidewall junction only.
    junc_params p2{.cjswd{2e-6}, .cjsws{2e-6}, .pbswd{4.0}, .pbsws{1.0}};
