...
static double run_case(double acm) noexcept
{
    ...
    // capMod=1 uses the Meyer path; acm toggles whether overlap is stamped as fixed caps (acm=0)
    // or embedded into Q(V) (acm!=0). Results should match.
    (void)m1->ptr->set_attribute(idx_capmod, {.d{1.0}, .type{::phy_engine::model::variant_type::d}});
