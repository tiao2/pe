...
static double run_case(bool drive_drain, double jsd, double jss) noexcept
{
    ...
    auto const idx_jss = find_attr(m1, "jss");
    ...
    if(idx_kp == SIZE_MAX || idx_is == SIZE_MAX || idx_n == SIZE_MAX || idx_js == SIZE_MAX || idx_jsw == SIZE_MAX || idx_jswg == SIZE_MAX ||
       idx_jsd == SIZE_MAX || idx_jss == SIZE_MAX || idx_ad == SIZE_MAX || idx_as == SIZE_MAX || idx_pd == SIZE_MAX || idx_ps == SIZE_MAX)
    {
        return std::numeric_limits<double>::quiet_NaN();
    }

    // Isolate bottom-area scaling, and disable channel current.
    ...
    (void)m1->ptr->set_attribute(idx_jsd, {.d{jsd}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_jss, {.d{jss}, .type{::phy_engine::model::variant_type::d}});
...
int main()
{
    // Larger jsd than jss should make the drain diode conduct more than the source diode, under identical geometry.
    double const id = run_case(true, 1e-2, 1e-3);
    double const is = run_case(false, 1e-2, 1e-3);
