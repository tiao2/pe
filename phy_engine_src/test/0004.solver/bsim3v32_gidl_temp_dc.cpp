...
static double run_case(double temp_c) noexcept
{
    ...
    if(idx_w == SIZE_MAX || idx_l == SIZE_MAX || idx_kp == SIZE_MAX || idx_diode_is == SIZE_MAX || idx_tnom == SIZE_MAX || idx_temp == SIZE_MAX ||
       idx_agidl == SIZE_MAX || idx_bgidl == SIZE_MAX || idx_cgidl == SIZE_MAX || idx_egidl == SIZE_MAX)
    {
    ...
    (void)m1->ptr->set_attribute(idx_bgidl, {.d{10.0}, .type{::phy_engine::model::variant_type::d}});
    ...
    (void)m1->ptr->set_attribute(idx_agidl, {.d{0.05}, .type{::phy_engine::model::variant_type::d}});
...
int main()
{
    double const vd_nom = run_case(27.0);
    double const vd_hot = run_case(127.0);
    if(!std::isfinite(vd_nom) || !std::isfinite(vd_hot)) { return 1; }

    // With barrier scaling enabled, higher temperature reduces the effective barrier and increases leakage.
    if(!(vd_hot < vd_nom - 0.05)) { return 2; }
    return 0;
}
