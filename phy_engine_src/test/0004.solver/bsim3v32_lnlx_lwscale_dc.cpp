...
static double run_case(double lref_m) noexcept
{
    ...
    auto const idx_lnlx = find_attr(m1, "lnlx");
    ...
    if(idx_w == SIZE_MAX || idx_l == SIZE_MAX || idx_lref == SIZE_MAX || idx_wref == SIZE_MAX || idx_kp == SIZE_MAX || idx_vth0 == SIZE_MAX ||
       idx_phi == SIZE_MAX || idx_k1 == SIZE_MAX || idx_k2 == SIZE_MAX || idx_dsub == SIZE_MAX || idx_dvt0 == SIZE_MAX || idx_ua == SIZE_MAX ||
       idx_ub == SIZE_MAX || idx_uc == SIZE_MAX || idx_lambda == SIZE_MAX || idx_pclm == SIZE_MAX || idx_nlx == SIZE_MAX || idx_lnlx == SIZE_MAX ||
       idx_k3 == SIZE_MAX || idx_w0 == SIZE_MAX)
    {
    ...
    // Isolate dvth_nlx effect.
    (void)m1->ptr->set_attribute(idx_k1, {.d{0.5}, .type{::phy_engine::model::variant_type::d}});
    ...
    // nlx_eff = lnlx*(L-lref). With lnlx=1 and dl=0.5um => nlx_eff=0.5um -> larger dvth_nlx -> smaller Id.
    (void)m1->ptr->set_attribute(idx_lnlx, {.d{1.0}, .type{::phy_engine::model::variant_type::d}});
...
int main()
{
    ...
    // Smaller lref => larger nlx_eff => larger dvth_nlx => smaller |Id|
