...
static double run_case(double temp_c) noexcept
{
    ...
    auto const idx_ua1 = find_attr(m1, "ua1");
    if(idx_kp == SIZE_MAX || idx_vth0 == SIZE_MAX || idx_tnom == SIZE_MAX || idx_ute == SIZE_MAX || idx_ua == SIZE_MAX || idx_ua1 == SIZE_MAX)
    {
        return std::numeric_limits<double>::quiet_NaN();
    }

    // Isolate the ua1 temperature effect:
    // - Disable UTE mobility scaling.
    // - Set ua=0 at TNOM and ua1>0 so ua increases with temperature.
    ...
    (void)m1->ptr->set_attribute(idx_ua1, {.d{0.01}, .type{::phy_engine::model::variant_type::d}});  // +1.0 at 127C
...
int main()
{
    ...
    // With ua1>0, mobility degrades more at higher temperature -> less current -> higher drain voltage through the pull-up.
