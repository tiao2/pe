...
static double run_case(bool drive_drain) noexcept
{
    ...
    auto const idx_nd = find_attr(m1, "diode_Nd");
    auto const idx_ns = find_attr(m1, "diode_Ns");
    auto const idx_js = find_attr(m1, "js");
    auto const idx_jsw = find_attr(m1, "jsw");
    auto const idx_jswg = find_attr(m1, "jswg");
    if(idx_kp == SIZE_MAX || idx_is == SIZE_MAX || idx_n == SIZE_MAX || idx_nd == SIZE_MAX || idx_ns == SIZE_MAX || idx_js == SIZE_MAX ||
       idx_jsw == SIZE_MAX || idx_jswg == SIZE_MAX)
    {
        return std::numeric_limits<double>::quiet_NaN();
    }

    // Disable channel current and geometry-current scaling; rely on diode_Nd/diode_Ns only.
    ...
    // Keep diode current small enough to avoid load-limited behavior.
    (void)m1->ptr->set_attribute(idx_is, {.d{1e-17}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_n, {.d{2.0}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_nd, {.d{1.0}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_ns, {.d{2.0}, .type{::phy_engine::model::variant_type::d}});
...
int main()
{
    // Lower ideality factor => larger forward current at the same bias. (Nd=1 vs Ns=2)
    double const id = run_case(true);
    double const is = run_case(false);
