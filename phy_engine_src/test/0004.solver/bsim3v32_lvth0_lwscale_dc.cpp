...
static double run_case(double L_m) noexcept
{
    ...
    // VG around threshold to make Vth shifts observable.
    ...
    auto const idx_lvth0 = find_attr(m1, "lvth0");
    auto const idx_kp = find_attr(m1, "Kp");
    if(idx_l == SIZE_MAX || idx_lref == SIZE_MAX || idx_lvth0 == SIZE_MAX || idx_kp == SIZE_MAX) { return std::numeric_limits<double>::quiet_NaN(); }
    ...
    // lvth0 unit assumed V/m so that lvth0*(L-Lref) gives volts.
    (void)m1->ptr->set_attribute(idx_lvth0, {.d{1e5}, .type{::phy_engine::model::variant_type::d}}); // 0.1V per +1um
    // Boost current a bit.
...
int main()
{
    // With lvth0>0, longer L => higher Vth => less current => drain droops less => higher drain voltage.
