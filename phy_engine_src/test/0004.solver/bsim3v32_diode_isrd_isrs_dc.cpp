...
static double run_case(bool drive_drain, double isrd, double isrs) noexcept
{
    ...
    // Bulk at +0.7V to forward-bias the diode under test.
    ...
    auto const idx_isrd = find_attr(m1, "diode_Isrd");
    auto const idx_isrs = find_attr(m1, "diode_Isrs");
    ...
    if(idx_kp == SIZE_MAX || idx_is == SIZE_MAX || idx_isr == SIZE_MAX || idx_isrd == SIZE_MAX || idx_isrs == SIZE_MAX || idx_nr == SIZE_MAX ||
       idx_js == SIZE_MAX || idx_jsw == SIZE_MAX || idx_jswg == SIZE_MAX)
    {
        return std::numeric_limits<double>::quiet_NaN();
    }

    // Disable channel current and geometry-current scaling; use recombination current only.
    ...
    (void)m1->ptr->set_attribute(idx_isr, {.d{0.0}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_isrd, {.d{isrd}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_isrs, {.d{isrs}, .type{::phy_engine::model::variant_type::d}});
...
int main()
{
    // Larger diode_Isrd than diode_Isrs should make the drain-side recombination current larger than the source-side.
    double const id = run_case(true, 1e-12, 1e-15);
    double const is = run_case(false, 1e-12, 1e-15);
