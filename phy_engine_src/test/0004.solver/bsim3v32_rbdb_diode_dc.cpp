...
static double run_case(double rbdb_ohm) noexcept
{
    ...
    if(idx_kp == SIZE_MAX || idx_is == SIZE_MAX || idx_n == SIZE_MAX || idx_rbdb == SIZE_MAX)
    {
...
int main()
{
    ...
    // With a large rbdb, the local body node near the drain diode rises under forward current,
    // reducing the effective forward bias and therefore reducing current.
    if(!(i0 > 1e-9)) { return 2; }
