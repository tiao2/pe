...
static double run_case(bool drive_drain, double jsrd, double jsrs) noexcept
{
    ...
    auto const idx_jsrd = find_attr(m1, "jsrd");
    auto const idx_jsrs = find_attr(m1, "jsrs");
    ...
    if(idx_kp == SIZE_MAX || idx_is == SIZE_MAX || idx_isr == SIZE_MAX || idx_n == SIZE_MAX || idx_nr == SIZE_MAX || idx_js == SIZE_MAX ||
       idx_jsw == SIZE_MAX || idx_jswg == SIZE_MAX || idx_jsr == SIZE_MAX || idx_jsrd == SIZE_MAX || idx_jsrs == SIZE_MAX || idx_ad == SIZE_MAX ||
       idx_as == SIZE_MAX || idx_pd == SIZE_MAX || idx_ps == SIZE_MAX)
    {
        return std::numeric_limits<double>::quiet_NaN();
    }

    // Disable channel and diffusion (Is); isolate recombination (Isr) via jsr* densities.
    ...
    (void)m1->ptr->set_attribute(idx_jsrd, {.d{jsrd}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_jsrs, {.d{jsrs}, .type{::phy_engine::model::variant_type::d}});
