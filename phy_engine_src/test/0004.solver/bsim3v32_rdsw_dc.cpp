...
static double run_case(double rdsw_ohm_m) noexcept
{
    ...
    if(idx_rdsw == SIZE_MAX) { return std::numeric_limits<double>::quiet_NaN(); }
    (void)m1->ptr->set_attribute(idx_rdsw, {.d{rdsw_ohm_m}, .type{::phy_engine::model::variant_type::d}});
...
int main()
{
    // rdsw is Ohm*m. With default W=1e-6, rdsw=1e-3 => ~1k Ohm effective, should noticeably reduce Id.
    double const vd_no = run_case(0.0);
    double const vd_hi = run_case(1e-3);

    if(!std::isfinite(vd_no) || !std::isfinite(vd_hi)) { return 1; }

    // Higher rdsw => less current => drain droops less through the load resistor => higher drain voltage.
    if(!(vd_hi > vd_no + 1e-2)) { return 2; }
    return 0;
}
