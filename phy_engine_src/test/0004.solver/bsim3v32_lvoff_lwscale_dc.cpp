...
static double run_case(double lref_m) noexcept
{
    ...
    auto const idx_lvoff = find_attr(m1, "lvoff");

    if(idx_w == SIZE_MAX || idx_l == SIZE_MAX || idx_lref == SIZE_MAX || idx_wref == SIZE_MAX || idx_kp == SIZE_MAX || idx_vth0 == SIZE_MAX ||
       idx_phi == SIZE_MAX || idx_k1 == SIZE_MAX || idx_k2 == SIZE_MAX || idx_dsub == SIZE_MAX || idx_dvt0 == SIZE_MAX || idx_nlx == SIZE_MAX ||
       idx_k3 == SIZE_MAX || idx_w0 == SIZE_MAX || idx_ua == SIZE_MAX || idx_ub == SIZE_MAX || idx_uc == SIZE_MAX || idx_lambda == SIZE_MAX ||
       idx_pclm == SIZE_MAX || idx_voff == SIZE_MAX || idx_lvoff == SIZE_MAX)
    {
    ...
    // Smaller lref => larger dl => larger voff_eff => smaller |Id|.
    (void)m1->ptr->set_attribute(idx_lvoff, {.d{1e5}, .type{::phy_engine::model::variant_type::d}});
