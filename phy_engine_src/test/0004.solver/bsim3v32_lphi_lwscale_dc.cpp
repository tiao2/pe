...
static double run_case(double lref_m) noexcept
{
    ...
    auto const idx_lphi = find_attr(m1, "lphi");
    ...
    if(idx_w == SIZE_MAX || idx_l == SIZE_MAX || idx_lref == SIZE_MAX || idx_wref == SIZE_MAX || idx_kp == SIZE_MAX || idx_vth0 == SIZE_MAX ||
       idx_phi == SIZE_MAX || idx_lphi == SIZE_MAX || idx_gamma == SIZE_MAX || idx_k1 == SIZE_MAX || idx_k2 == SIZE_MAX || idx_dsub == SIZE_MAX ||
       idx_dvt0 == SIZE_MAX || idx_dvt1 == SIZE_MAX || idx_dvt2 == SIZE_MAX || idx_eta0 == SIZE_MAX || idx_etab == SIZE_MAX ||
       idx_nlx == SIZE_MAX || idx_k3 == SIZE_MAX || idx_w0 == SIZE_MAX || idx_ua == SIZE_MAX || idx_ub == SIZE_MAX || idx_uc == SIZE_MAX ||
       idx_lambda == SIZE_MAX || idx_pclm == SIZE_MAX || idx_voff == SIZE_MAX || idx_nfactor == SIZE_MAX || idx_cit == SIZE_MAX ||
       idx_keta == SIZE_MAX)
    {
    ...
    // Isolate lphi effect (phi_eff increases as lref decreases, so |Id| should decrease in subthreshold).
    (void)m1->ptr->set_attribute(idx_lphi, {.d{1e6}, .type{::phy_engine::model::variant_type::d}});  // V/m
