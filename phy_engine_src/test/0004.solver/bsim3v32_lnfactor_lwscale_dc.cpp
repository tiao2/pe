...
static double run_case(double lref_m) noexcept
{
    ...
    auto const idx_lnfactor = find_attr(m1, "lnfactor");
    ...
    if(idx_w == SIZE_MAX || idx_l == SIZE_MAX || idx_lref == SIZE_MAX || idx_wref == SIZE_MAX || idx_kp == SIZE_MAX || idx_vth0 == SIZE_MAX ||
       idx_nfactor == SIZE_MAX || idx_lnfactor == SIZE_MAX || idx_dsub == SIZE_MAX || idx_ua == SIZE_MAX || idx_ub == SIZE_MAX || idx_uc == SIZE_MAX ||
       idx_keta == SIZE_MAX)
    {
    ...
    // lnfactor*(L-lref): with lnfactor=1e6 and dl=0.5um => nfactor_eff=0.5 (n increases, Id increases).
    (void)m1->ptr->set_attribute(idx_lnfactor, {.d{1e6}, .type{::phy_engine::model::variant_type::d}});
