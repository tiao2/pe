...
static double run_case(double is_alias, double n_alias) noexcept
{
    ...
    // Bulk at +0.7V to forward-bias the drain diode (source tied to bulk).
    ...
    // Drain node returned to ground through a resistor.
    ...
    auto const idx_is = find_attr(m1, "is");
    auto const idx_n = find_attr(m1, "n");
    ...
    // Disable channel and geometry scaling; use is/n aliases only.
    ...
    (void)m1->ptr->set_attribute(idx_is, {.d{is_alias}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_n, {.d{n_alias}, .type{::phy_engine::model::variant_type::d}});
...
int main()
{
    double const i1 = run_case(1e-14, 1.0);
    double const i2 = run_case(1e-16, 2.0);
    if(!std::isfinite(i1) || !std::isfinite(i2)) { return 1; }
    if(!(i1 > 1e-12) || !(i2 > 1e-12)) { return 2; }
    if(!(i1 > i2 * 100.0)) { return 3; }
    return 0;
}
