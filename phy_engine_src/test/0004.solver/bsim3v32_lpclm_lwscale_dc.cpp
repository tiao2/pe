...
static double run_case(double lref_m) noexcept
{
    ...
    // Force using BSIM3-style CLM rather than legacy lambda hook.
    (void)m1->ptr->set_attribute(idx_lambda, {.d{0.0}, .type{::phy_engine::model::variant_type::d}});

    // Increase dl by decreasing lref; with lpclm > 0 this increases pclm_eff and strengthens CLM -> larger Id.
    (void)m1->ptr->set_attribute(idx_pclm, {.d{0.5}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_lpclm, {.d{1e6}, .type{::phy_engine::model::variant_type::d}});
