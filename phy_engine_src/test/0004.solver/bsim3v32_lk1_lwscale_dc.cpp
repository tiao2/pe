...
static double run_case(double lref_m) noexcept
{
    ...
    auto const idx_lk1 = find_attr(m1, "lk1");
    ...
    if(idx_w == SIZE_MAX || idx_l == SIZE_MAX || idx_lref == SIZE_MAX || idx_wref == SIZE_MAX || idx_kp == SIZE_MAX || idx_vth0 == SIZE_MAX ||
       idx_k1 == SIZE_MAX || idx_lk1 == SIZE_MAX || idx_k2 == SIZE_MAX || idx_dsub == SIZE_MAX || idx_dvt0 == SIZE_MAX || idx_ua == SIZE_MAX ||
       idx_ub == SIZE_MAX || idx_uc == SIZE_MAX || idx_lambda == SIZE_MAX || idx_pclm == SIZE_MAX)
    {
    ...
    // Isolate body-effect K1 scaling.
    ...
    // Larger dl (via smaller lref) => larger k1_eff => higher Vth under body bias => smaller Id.
    (void)m1->ptr->set_attribute(idx_lk1, {.d{1e6}, .type{::phy_engine::model::variant_type::d}});
