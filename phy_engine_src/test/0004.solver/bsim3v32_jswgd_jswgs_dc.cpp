...
static double run_case(bool drive_drain, double jswgd, double jswgs) noexcept
{
    ...
    auto const idx_jswgd = find_attr(m1, "jswgd");
    auto const idx_jswgs = find_attr(m1, "jswgs");
    ...
    if(idx_kp == SIZE_MAX || idx_is == SIZE_MAX || idx_n == SIZE_MAX || idx_w == SIZE_MAX || idx_js == SIZE_MAX || idx_jsw == SIZE_MAX ||
       idx_jswg == SIZE_MAX || idx_jswgd == SIZE_MAX || idx_jswgs == SIZE_MAX || idx_ad == SIZE_MAX || idx_as == SIZE_MAX || idx_pd == SIZE_MAX ||
       idx_ps == SIZE_MAX)
    {
        return std::numeric_limits<double>::quiet_NaN();
    }

    // Isolate gate-edge (Weff) scaling, and disable channel current.
    ...
    (void)m1->ptr->set_attribute(idx_jswgd, {.d{jswgd}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_jswgs, {.d{jswgs}, .type{::phy_engine::model::variant_type::d}});
