...
static double run_case(bool use_source_diode, bool enable_breakdown) noexcept
{
    ...
    auto const idx_bv = find_attr(m1, "bv");
    auto const idx_ibv = find_attr(m1, "ibv");
    if(idx_kp == SIZE_MAX || idx_is == SIZE_MAX || idx_n == SIZE_MAX || idx_bv == SIZE_MAX || idx_ibv == SIZE_MAX)
    {
    ...
    (void)m1->ptr->set_attribute(idx_bv, {.d{enable_breakdown ? 1.0 : 0.0}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_ibv, {.d{enable_breakdown ? 1e-9 : 0.0}, .type{::phy_engine::model::variant_type::d}});
...
int main()
{
    // Using bv/ibv aliases should enable breakdown for both drain and source junctions simultaneously.
