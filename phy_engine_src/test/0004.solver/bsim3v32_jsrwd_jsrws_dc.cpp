...
static double run_case(bool drive_drain, double jsrwd, double jsrws) noexcept
{
    ...
    auto const idx_jsrwd = find_attr(m1, "jsrwd");
    auto const idx_jsrws = find_attr(m1, "jsrws");
    ...
    if(idx_kp == SIZE_MAX || idx_is == SIZE_MAX || idx_isr == SIZE_MAX || idx_n == SIZE_MAX || idx_nr == SIZE_MAX || idx_js == SIZE_MAX ||
       idx_jsw == SIZE_MAX || idx_jswg == SIZE_MAX || idx_jsr == SIZE_MAX || idx_jsrwd == SIZE_MAX || idx_jsrws == SIZE_MAX || idx_ad == SIZE_MAX ||
    ...
    (void)m1->ptr->set_attribute(idx_jsrwd, {.d{jsrwd}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_jsrws, {.d{jsrws}, .type{::phy_engine::model::variant_type::d}});
