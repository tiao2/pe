...
static double run_case(double mobmod) noexcept
{
    ...
    // Choose ua in the "mobMod=1 units" so the effect is observable.
    (void)m1->ptr->set_attribute(idx_ua, {.d{1.0}, .type{::phy_engine::model::variant_type::d}});
...
int main()
{
    double const id_mob0 = run_case(0.0);
    double const id_mob1 = run_case(1.0);

    if(!std::isfinite(id_mob0) || !std::isfinite(id_mob1)) { return 1; }

    double const a0 = std::abs(id_mob0);
    double const a1 = std::abs(id_mob1);
    if(!(a0 > 1e-12)) { return 2; }

    // mobMod=1 should reduce mobility compared to mobMod=0 (constant mobility).
    if(!(a1 < a0 * 0.9)) { return 3; }
    if(!(a1 > a0 * 0.2)) { return 4; }
    return 0;
}
