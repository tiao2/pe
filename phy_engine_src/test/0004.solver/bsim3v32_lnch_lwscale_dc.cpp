...
static double run_case(double lref_m) noexcept
{
    ...
    auto const idx_nch = find_attr(m1, "nch");
    auto const idx_lnch = find_attr(m1, "lnch");
    ...
    if(idx_w == SIZE_MAX || idx_l == SIZE_MAX || idx_lref == SIZE_MAX || idx_wref == SIZE_MAX || idx_kp == SIZE_MAX || idx_vth0 == SIZE_MAX ||
       idx_phi == SIZE_MAX || idx_nch == SIZE_MAX || idx_lnch == SIZE_MAX || idx_k1 == SIZE_MAX || idx_k2 == SIZE_MAX || idx_nlx == SIZE_MAX ||
       idx_k3 == SIZE_MAX || idx_w0 == SIZE_MAX || idx_dvt0 == SIZE_MAX || idx_dsub == SIZE_MAX || idx_ua == SIZE_MAX || idx_ub == SIZE_MAX ||
       idx_uc == SIZE_MAX || idx_voff == SIZE_MAX)
    {
    ...
    // Isolate nch effect: remove phi-dependent Vth terms and other effects.
    ...
    (void)m1->ptr->set_attribute(idx_nch, {.d{1e23}, .type{::phy_engine::model::variant_type::d}});
    // Smaller lref => larger dl => larger nch_eff => smaller xdep => larger cd => larger n => larger |Id| (subthreshold).
    (void)m1->ptr->set_attribute(idx_lnch, {.d{1e29}, .type{::phy_engine::model::variant_type::d}});
