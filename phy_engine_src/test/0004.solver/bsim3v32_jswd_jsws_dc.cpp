...
static double run_case(bool drive_drain, double jswd, double jsws) noexcept
{
    ...
    auto const idx_jswd = find_attr(m1, "jswd");
    auto const idx_jsws = find_attr(m1, "jsws");
    ...
    if(idx_kp == SIZE_MAX || idx_is == SIZE_MAX || idx_n == SIZE_MAX || idx_js == SIZE_MAX || idx_jsw == SIZE_MAX || idx_jswg == SIZE_MAX ||
       idx_jswd == SIZE_MAX || idx_jsws == SIZE_MAX || idx_ad == SIZE_MAX || idx_as == SIZE_MAX || idx_pd == SIZE_MAX || idx_ps == SIZE_MAX)
    {
        return std::numeric_limits<double>::quiet_NaN();
    }

    // Isolate perimeter scaling, and disable channel current.
    ...
    (void)m1->ptr->set_attribute(idx_jswd, {.d{jswd}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_jsws, {.d{jsws}, .type{::phy_engine::model::variant_type::d}});
...
int main()
{
    double const id = run_case(true, 1e-7, 1e-8);
    double const is = run_case(false, 1e-7, 1e-8);
