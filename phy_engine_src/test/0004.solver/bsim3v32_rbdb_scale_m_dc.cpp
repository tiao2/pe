...
static double run_case(double m_mult, double rbdb_ohm) noexcept
{
    ...
    add_to_node(nl, *m1, 3, nl.ground_node);  // B (must be connected for rbdb to allocate)
    ...
    if(idx_m == SIZE_MAX || idx_rbdb == SIZE_MAX || idx_kp == SIZE_MAX || idx_is == SIZE_MAX)
    {
    ...
    // Apply a reverse-bias voltage to keep the drain-body diode off; current then flows through rbdb only.
    // V(nbd) = -1V wrt bulk(0V) => I ≈ 1 / rbdb_eff.
...
int main()
{
    // rbdb scales like other series resistances: rbdb_eff = rbdb / (m*nf)
    // Choose values such that rbdb_eff matches between cases.
