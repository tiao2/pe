...
static double run_case(double lref_m) noexcept
{
    ...
    auto const idx_leta0 = find_attr(m1, "leta0");
    auto const idx_kp = find_attr(m1, "Kp");
    if(idx_l == SIZE_MAX || idx_lref == SIZE_MAX || idx_wref == SIZE_MAX || idx_dsub == SIZE_MAX || idx_eta0 == SIZE_MAX || idx_leta0 == SIZE_MAX ||
       idx_kp == SIZE_MAX)
    {
        return std::numeric_limits<double>::quiet_NaN();
    }

    // Fix L, vary lref so dl changes and isolates leta0.
    ...
    // Make theta_dibl large and stable.
    ...
    (void)m1->ptr->set_attribute(idx_leta0, {.d{1e5}, .type{::phy_engine::model::variant_type::d}}); // +0.05 per +0.5um
...
int main()
{
    // Smaller lref => larger dl => larger eta0_eff => more DIBL => more current => lower drain voltage.
