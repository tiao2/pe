...
static double run_case(double lref_m) noexcept
{
    ...
    // VDS=2V
    auto [vds_src, vds_pos] = add_model(nl, ::phy_engine::model::VDC{.V = 2.0});
    ...
    auto const idx_lprwg = find_attr(m1, "lprwg");

    if(idx_w == SIZE_MAX || idx_l == SIZE_MAX || idx_lref == SIZE_MAX || idx_wref == SIZE_MAX || idx_kp == SIZE_MAX || idx_vth0 == SIZE_MAX ||
       idx_lambda == SIZE_MAX || idx_pclm == SIZE_MAX || idx_dsub == SIZE_MAX || idx_rdsw == SIZE_MAX || idx_prwg == SIZE_MAX || idx_prwb == SIZE_MAX ||
       idx_lprwg == SIZE_MAX)
    {
    ...
    // Enable bias-dependent Rds: R ~ rdsw/Weff * (1 + prwg*Vgsteff + prwb*Vbseff).
    (void)m1->ptr->set_attribute(idx_rdsw, {.d{1e-3}, .type{::phy_engine::model::variant_type::d}}); // ~100 Ohm @ 10um
    ...
    // Larger dl (via smaller lref) => larger prwg_eff => larger Rds => smaller Id.
    (void)m1->ptr->set_attribute(idx_lprwg, {.d{1e6}, .type{::phy_engine::model::variant_type::d}});
...
int main()
{
    ...
    // Smaller lref => larger Rds => smaller |Id|
