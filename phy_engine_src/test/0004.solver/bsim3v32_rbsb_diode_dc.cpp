...
static double run_case(double rbsb_ohm) noexcept
{
    ...
    // Source fixed at -0.7V (forward-bias source-body diode for NMOS when bulk is at 0V).
    auto [vs_src, vs_pos] = add_model(nl, ::phy_engine::model::VDC{.V = -0.7});
    auto& n_source = create_node(nl);
    add_to_node(nl, *vs_src, 0, n_source);
    add_to_node(nl, *vs_src, 1, nl.ground_node);

    // Drain and gate at 0V (disable channel; isolate diode path).
    ...
    if(idx_kp == SIZE_MAX || idx_is == SIZE_MAX || idx_n == SIZE_MAX || idx_rbsb == SIZE_MAX)
    {
