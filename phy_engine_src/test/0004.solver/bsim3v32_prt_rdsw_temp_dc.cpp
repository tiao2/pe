...
static double run_case(double temp_c) noexcept
{
    ...
    auto const idx_ute = find_attr(m1, "ute");
    auto const idx_rdsw = find_attr(m1, "rdsw");
    auto const idx_prt = find_attr(m1, "prt");
    if(idx_kp == SIZE_MAX || idx_vth0 == SIZE_MAX || idx_tnom == SIZE_MAX || idx_ute == SIZE_MAX || idx_rdsw == SIZE_MAX || idx_prt == SIZE_MAX)
    {
        return std::numeric_limits<double>::quiet_NaN();
    }

    // Isolate the prt temperature effect (rdsw increases with temperature).
    (void)m1->ptr->set_attribute(idx_kp, {.d{1e-3}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_vth0, {.d{1.0}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_tnom, {.d{27.0}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_ute, {.d{0.0}, .type{::phy_engine::model::variant_type::d}});

    // rdsw is Ohm*m; with W=10um default => ~100 Ohm base contribution.
    (void)m1->ptr->set_attribute(idx_rdsw, {.d{1e-3}, .type{::phy_engine::model::variant_type::d}});
    (void)m1->ptr->set_attribute(idx_prt, {.d{0.01}, .type{::phy_engine::model::variant_type::d}});  // +100% at 127C
...
int main()
{
    ...
    // Higher temperature => higher rdsw => less effective Ids => higher drain voltage through the pull-up.
